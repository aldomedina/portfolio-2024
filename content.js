const featuredProjects = [
  {
    id: 'wargames',
    workingFor: 'FINE DIGITAL',
    year: 2023,
    title: 'wargames',
    url: 'https://res.cloudinary.com/dhrwv7wvb/video/upload/v1711930825/portfolio-2024/wargames_tledp8.mp4',
    imageUrl:
      'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930817/portfolio-2024/wargames-fallback_qnlk84.webp',
    ratio: 1.88,
    route: '/projects/wargames',
    tools: ['JAVASCRIPT', 'THREE.JS', 'R3F', 'WEB3.JS', 'GLSL', 'TYPESCRIPT', 'ZUSTAND'],
    images: {
      featured: {
        type: 'image',
        url: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930820/portfolio-2024/wargamesfinal_q4np4x.webp',
        width: 3024,
        height: 1728,
      },
      carousel: [
        {
          id: 'wargames1',
          image: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930817/portfolio-2024/wargames1_sp0i0h.webp',
          scale: [2.4, 2.4, 0.01],
        },
        {
          id: 'wargames2',
          image: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930818/portfolio-2024/wargames2_fmzvf2.webp',
          scale: [2.4, 2.4, 0.01],
        },
        {
          id: 'wargames3',
          image: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930818/portfolio-2024/wargames3_hgsxcx.webp',
          scale: [2.4, 2.4, 0.01],
        },
        {
          id: 'wargames4',
          image: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930819/portfolio-2024/wargames4_pb32oo.webp',
          scale: [2.4, 2.4, 0.01],
        },
        {
          id: 'wargames5',
          image: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930819/portfolio-2024/wargames5_ahbec6.webp',
          scale: [2.4, 2.4, 0.01],
        },
        {
          id: 'wargames6',
          image: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930820/portfolio-2024/wargames6_w1d3ns.webp',
          scale: [2.4, 2.4, 0.01],
        },
      ],
      final: {
        type: 'image',
        url: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930817/portfolio-2024/wargames-fallback_qnlk84.webp',
        width: 1536,
        height: 818,
      },
    },
    blurDataUrl:
      'data:image/webp;base64,',
  },
  {
    id: 'alumni-vis',
    workingFor: 'UNIVERSIDAD ADOLFO IBAÑEZ',
    year: 2023,
    title: 'alumni vis',
    url: 'https://res.cloudinary.com/dhrwv7wvb/video/upload/v1711930824/portfolio-2024/alumni_mkgylj.mp4',
    imageUrl:
      'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930808/portfolio-2024/alumni-fallback_pmig2m.webp',
    ratio: 0.966,
    route: '/projects/alumni-vis',
    tools: ['JAVASCRIPT', 'THREE.JS', 'R3F', 'GLSL'],
    images: {
      featured: {
        type: 'image',
        url: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930816/portfolio-2024/ui-opened_vid4fd.webp',
        width: 4536,
        height: 2946,
      },
      carousel: [],
      final: {
        type: 'image',
        url: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930816/portfolio-2024/ui-closed_v2urat.webp',
        width: 4536,
        height: 2946,
      },
    },
    blurDataUrl:
      'data:image/webp;base64,',
  },
  {
    id: 'solids',
    workingFor: 'FINE Digital',
    year: 2021,
    title: 'solids',
    url: 'https://res.cloudinary.com/dhrwv7wvb/video/upload/v1711930822/portfolio-2024/solids-ui-compressed_hzuphr.mp4',
    imageUrl:
      'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930808/portfolio-2024/solids-fallback_vj3nzl.webp',
    ratio: 1.6,
    route: '/projects/solids',
    tools: ['JAVASCRIPT', 'THREE.JS', 'R3F', 'WEB3.JS', 'TYPESCRIPT'],
    images: {
      featured: {
        type: 'video',
        url: 'https://res.cloudinary.com/dhrwv7wvb/video/upload/v1711930822/portfolio-2024/solids-ui-compressed_hzuphr.mp4',
        withFrame: true,
      },
      carousel: [
        {
          id: 'solids1594',
          image: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930808/portfolio-2024/solids1594_fokyry.webp',
          scale: [2.4, 2.4, 0.01],
          position: [1, 1, 1],
        },
        {
          id: 'solidsgrid12',
          image:
            'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930812/portfolio-2024/solidsgrid12_zlcibb.webp',
          scale: [2.4, 2.4, 0.01],
          position: [1, 1, 1],
        },

        {
          id: 'solidsgrid61',
          image:
            'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930811/portfolio-2024/solidsgrid6-1_xvh3uj.webp',
          scale: [2.4, 2.4, 0.01],
          position: [1, 1, 1],
        },
        {
          id: 'solids1475',
          image: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930808/portfolio-2024/solids1475_by99tr.webp',
          scale: [2.4, 2.4, 0.01],
          position: [1, 1, 1],
        },
        {
          id: 'solids8224',
          image: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930809/portfolio-2024/solids8224_orctw8.webp',
          scale: [2.4, 2.4, 0.01],
          position: [1, 1, 1],
        },
        {
          id: 'solidsgrid62',
          image:
            'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930811/portfolio-2024/solidsgrid6-2_rst2lw.webp',
          scale: [2.4, 2.4, 0.01],
          position: [1, 1, 1],
        },
      ],
      final: {
        type: 'image',
        url: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930811/portfolio-2024/solidscity_qbju8j.webp',
        width: 4000,
        height: 2207,
      },
    },
    blurDataUrl:
      'data:image/webp;base64,',
  },
  {
    id: 'teapot',
    workingFor: '0xFAR',
    year: 2022,
    title: 'UTAH’S ODISSEY',
    url: 'https://res.cloudinary.com/dhrwv7wvb/video/upload/v1711930822/portfolio-2024/teapot-5-comp_egyo5n.mp4',
    imageUrl:
      'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930813/portfolio-2024/teapot-fallback_pckyju.webp',
    ratio: 1,
    route: '/projects/teapot',
    tools: ['JAVASCRIPT', 'THREE.JS', 'GLSL'],
    images: {
      featured: {
        type: 'video',
        url: 'https://res.cloudinary.com/dhrwv7wvb/video/upload/v1711930830/portfolio-2024/teapotfeatured_fiavia.mp4',
      },
      carousel: [
        {
          id: 'teapot1',
          image: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930812/portfolio-2024/teapot1_bsscws.webp',
          scale: [2.4, 2.4, 0.01],
        },
        {
          id: 'teapot2',
          image: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930813/portfolio-2024/teapot2_kao3ws.webp',
          scale: [2.4, 2.4, 0.01],
        },
        {
          id: 'teapot3',
          image: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930814/portfolio-2024/teapot3_j8nuye.webp',
          scale: [2.4, 2.4, 0.01],
        },
        {
          id: 'teapot4',
          image: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930814/portfolio-2024/teapot4_wkvrah.webp',
          scale: [2.4, 2.4, 0.01],
        },
        {
          id: 'teapot5',
          image: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930815/portfolio-2024/teapot5_hjuuqp.webp',
          scale: [2.4, 2.4, 0.01],
        },
        {
          id: 'teapot6',
          image: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930815/portfolio-2024/teapot6_oplhtc.webp',
          scale: [2.4, 2.4, 0.01],
        },
      ],
      final: {
        type: 'image',
        url: 'https://res.cloudinary.com/dhrwv7wvb/image/upload/v1711930810/portfolio-2024/finalteapot_rxooyx.webp',
        width: 1728,
        height: 864,
      },
    },
    blurDataUrl:
      'data:image/webp;base64,',
  },
]

const otherProjects = [
  {
    id: 'Quasars',
    name: 'QUASARS',
    web: 'https://quantum.art/',
    endClient: 'QUANTUM ART',
    workingFor: 'FINE DIGITAL',
    roles: 'FRONT-END/DESIGNER',
    year: 2022,
    tools: ['React.js', 'Three.js', 'R3F', 'Sass', 'Web3.js', 'Typescript'],
    body: 'Design and development of an interactive digital space (Metaverse) for the exhibition of various digital artworks and photographs offered in auction by the company.',
  },
  {
    id: 'fine',
    name: 'FINE DIGITAL',
    endClient: 'FINE DIGITAL',
    workingFor: 'FINE DIGITAL',
    roles: 'FULL-STACK/DESIGNER',
    year: 2021,
    tools: ['Next.js', 'Three.js', 'Sass', 'Web3.js', 'Typescript', 'Sanity.io'],
    body: 'Design and development of Fine Digitals website. Fine, a startup established following the acquisition of funding from the a16z crypto venture capital fund, specializes in the development of collaborative art projects. These projects are designed for trading across multiple blockchain platforms.',
  },
  {
    id: 'TEGRA118',
    name: 'TEGRA118',
    body: 'Cloud marketplace for asset managers and traders. It provides them with portfolio management and trading capabilities. The platform provides a dynamic subdomain service for each entity and specific layouts for each user role.',
    endClient: 'jigsaw.xyz',
    workingFor: 'StudioGraphene',
    roles: 'FRONT-END',
    year: 2021,
    web: 'https://www.tegra118.com/',
    tools: ['React.js', 'Redux', 'Typescript', 'AWS Cognito', 'Styled Components', 'Material UI', 'Cypress', 'Jest'],
  },
  {
    id: 'Winnibook',
    name: 'WINNIBOOK',
    workingFor: 'independent',
    body: 'Manitoba’s (Canada) directory of local businesses. It seeks to increase local business visibility through an online directory and blog posts about them, powered by a custom search engine',
    endClient: 'WINNIBOOK',
    roles: 'FRONT-END/DESIGNER',
    year: 2021,
    tools: ['Next.js', 'Tailwind CSS', 'GraphQL'],
  },
  {
    id: 'HELLOSOLO',
    name: 'HELLOSOLO',
    body: 'Platform that allows users to browse and purchase tickets to dating events. Users who attend can then match with other users and start communicating through the platform. Organisations can create accounts to host and manage their own events.',
    endClient: 'hellosolo',
    workingFor: 'StudioGraphene',
    roles: 'FRONT-END',
    year: 2020,
    tools: ['Nuxt.js', 'Vue.js', 'Sass', 'Stripe'],
  },
  {
    id: 'Hatch',
    name: 'HATCH AND CREATE',
    body: 'Web application for developing and managing design cases for various products that are sold internally through Hatch, Shopify or Etsy platform. It also has a community section that it’s handled through drupal.',
    endClient: 'Fine Digital',
    workingFor: 'StudioGraphene',
    roles: 'FRONT-END',
    year: 2020,
    tools: ['React.js', 'Sass', 'Redux', 'Stripe'],
    web: 'https://www.hatch.global/',
  },
  {
    id: 'GOLEARN',
    name: 'GOLEARN',
    body: 'AI intervention and assessment platform to help improve the English literacy and learning outcomes of students. It provides a diverse range of exercises, follow up tools and a class based system for teachers and schools.',
    endClient: 'Golearn',
    workingFor: 'StudioGraphene',
    roles: 'FRONT-END',
    year: 2020,
    tools: ['React.js', 'Sass', 'Redux'],
    web: 'https://portal.golearn.guru/login',
  },
  {
    id: 'Extrai',
    name: 'EXTRAI',
    body: 'Design of the visual identity for the EXTRAI Art Festival in Lousal, Portugal, in collaboration with a collective of freelance designers. My role involved creating graphic pieces, developing the website, and producing interactive 3D graphics.',
    endClient: 'Extrai Art Festival',
    workingFor: 'Independent',
    roles: 'FRONT-END/DESIGNER',
    year: 2019,
    tools: ['Gatsby.js', 'Styled Components'],
  },
  {
    id: 'Post-ism',
    name: 'POST-ISM',
    body: 'Website for the Post-ism design exhibition, featuring projects developed during the course of the Master’s Degree in Communication Design (Fine Arts, ULisboa), addressing contemporary methods of communication, production, and dissemination of knowledge.',
    endClient: 'Extrai Art Festival',
    workingFor: 'Independent',
    roles: 'FRONT-END/DESIGNER',
    year: 2019,
    tools: ['Vanilla js', 'Three.js'],
    web: 'https://post-ism.fbaul-dcnm.pt/',
  },
]

export { featuredProjects, otherProjects }
